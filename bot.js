...
client.on('guildMemberAdd', member => {
     ...
     const welcomer =  member.guild.channels.get("465552810298310657");
    if(!welcomer) return;
      if(welcomer) {
         moment.locale('ar-ly');
         var m = member.user;
...
client.on("ready", () => {
    var guild;
    while (!guild)
        guild = client.guilds.get("294805996893896705");
...
client.on("guildMemberAdd", (member) => {
    let channel = member.guild.channels.get("465552810298310657");
    if (!channel) {
